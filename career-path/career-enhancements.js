...
(function() {
  ...
  function createLinkedInHTML(roleTitle) {
    ...
    return `<div class="s2i-linkedin-wrap s2i-enhancement">
      <a href="${url}" target="_blank" rel="noopener" class="s2i-linkedin-btn">
        ${LINKEDIN_SVG} Vagas no LinkedIn
      </a>
      <span class="s2i-linkedin-note">Limitado às vagas atualmente publicadas</span>
    </div>`;
  }
  ...
  function createFreeCourseHTML(skills) {
    ...
    for (const skill of skills) {
      ...
      for (const c of courses) {
        ...
        chipsHTML += `<a href="${url}" target="_blank" rel="noopener" class="s2i-course-chip" title="Pesquisar ${c.subtopic} em ${p.name}">
          <span class="s2i-chip-icon">${p.icon}</span>
          <span>
            <span class="s2i-chip-name">${p.name}</span>
            <span class="s2i-chip-sub"> · ${c.subtopic}</span>
            <span class="s2i-chip-free">Gratuito</span>
          </span>
        </a>`;
      }
    ...
    return `<div class="s2i-free-courses s2i-enhancement">
      <div class="s2i-free-courses-title">🎓 Formações gratuitas recomendadas</div>
      ${groupsHTML}
    </div>`;
  }
  ...
  function processResults() {
    ...
    const hasResults = pageText.includes('PRÓXIMOS CARGOS RECOMENDADOS') || 
                       pageText.includes('CARGOS RECOMENDADOS') ||
    ...
    for (const el of allElements) {
      ...
      if (text.match(/^\d+%\s*fit$/i)) {
        ...
        for (let i = 0; i < 10 && container; i++) {
          ...
          if (containerText.includes('JÁ TENS') && containerText.includes('PRECISAS')) {
    ...
    for (const { container } of roleCards) {
      ...
      for (const h of headings) {
        ...
        if (t.length > 10 && !t.includes('JÁ TENS') && !t.includes('PRECISAS') && 
            !t.includes('FORMAÇÕES') && !t.includes('CERTIFICAÇÕES') &&
      ...
      const precisasEl = findElementByText(container, ['PRECISAS'], null);
      if (precisasEl) {
        ...
        for (const line of lines) {
          ...
          if (clean && clean.length > 5 && !clean.includes('JÁ TENS') && !clean.includes('FORMAÇÕES')) {
      ...
      if (precisasSkills.length > 0) {
        ...
        const formacoesEl = findElementByText(container, ['FORMAÇÕES RECOMENDADAS', 'FORMAÇÕES'], null);
        if (formacoesEl) {
