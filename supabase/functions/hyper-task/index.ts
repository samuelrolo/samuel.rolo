...
async function enrichWithCompanyData(cvText: string, language: string): Promise<string> {
  ...
  if (mainCompany) {
    ...
    if (mainCompany.description) context += `${isEN ? 'Description' : 'Descricao'}: ${mainCompany.description.substring(0, 300)}\n`;

    if (mainCompany.industry) context += `${isEN ? 'Industry' : 'Industria'}: ${mainCompany.industry}\n`;

    if (mainCompany.employee_count != null) context += `${isEN ? 'Employees' : 'Funcionarios'}: ${mainCompany.employee_count.toLocaleString()}\n`;

    if (mainCompany.specialties?.length) context += `${isEN ? 'Specialties' : 'Especialidades'}: ${mainCompany.specialties.join(', ')}\n`;

    if (mainCompany.hq) context += `${isEN ? 'HQ' : 'Sede'}: ${mainCompany.hq}\n`;

    if (mainCompany.founded_year) context += `${isEN ? 'Founded' : 'Fundada'}: ${mainCompany.founded_year}\n`;

    if (competitors.length > 0) {

      context += `${isEN ? 'Main competitors' : 'Principais concorrentes'}: ${competitors.join(', ')}\n`;
  ...
  for (const comp of otherCompanies) {
    ...
    context += `\n${isEN ? 'PREVIOUS EMPLOYER' : 'EMPREGADOR ANTERIOR'}: ${comp.name}\n`;

    if (comp.industry) context += `${isEN ? 'Industry' : 'Industria'}: ${comp.industry}\n`;

    if (comp.employee_count != null) context += `${isEN ? 'Employees' : 'Funcionarios'}: ${comp.employee_count.toLocaleString()}\n`;

    if (comp.specialties?.length) context += `${isEN ? 'Specialties' : 'Especialidades'}: ${comp.specialties.join(', ')}\n`;
...
- OBLIGATORIO: strategic_comparison DEBE tener exactamente 3 ítems correspondientes a los 3 strategic_paths, cada uno con: path_name, success_probability, estimated_time, effort_level, risk_level, salary_impact, profile_fit
- OBLIGATORIO: tradeoffs DEBE tener exactamente 3 ítems correspondientes a los 3 strategic_paths, cada uno con: path_name, you_gain, you_give_up, hidden_risk, real_scenario
      ...
      const htmlContent = `
        ...
        <div style="font-family:'Segoe UI',Arial,sans-serif;max-width:680px;margin:0 auto;padding:30px 20px;background:#fafaf8">
          ...
          <div style="background:#fff;border-radius:12px;padding:30px;border:1px solid #e8e4dc">
            ...
            <h2 style="color:#1a1a1a;font-size:22px;margin:0 0 8px">${isEN ? typeLabel.en : typeLabel.pt}</h2>
