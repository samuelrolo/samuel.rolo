...
16. ATS KEYWORDS BY FIELD: The keywords detected and recommended MUST be relevant to the professional field. A nurse needs "patient care, clinical assessment, medication administration" — NOT "stakeholder management, ROI, KPIs". A mechanic needs "diagnostics, preventive maintenance, hydraulic systems" — NOT "project management, agile methodology".` : `Analisa o seguinte CV e retorna um JSON EXACTAMENTE com esta estrutura. CADA campo deve conter análise ESPECÍFICA ao CV — cita secções, frases e dados concretos do CV.
          ...
          body: JSON.stringify({
            contents: [
              ...
              },
              {
                ...
                parts: [
                  {
                    text: isEN ? 'Understood. I will analyse the CV and return a complete JSON analysis.' : 'Compreendido. Vou analisar o CV e retornar JSON com análise completa.'
                  }
                ]
              },
              {
...
- NEVER dump raw data — weave it naturally into the conversation.${chatCompanyContext}${profileContext}`;
        ...
        const modelAck = isPT ? 'Compreendido. Sou o Career Advisory da Share2Inspire. Estou pronto para ajudar.' : 'Understood. I am the Career Advisory by Share2Inspire. Ready to help.';
        const contents = [
          {
            role: 'user',
            parts: [
...
- Retorna APENAS o JSON, sem texto adicional`;
    ...
    if (mode === 'linkedin_roast') {
      ...
      if (!cvText || cvText.length < 50) {
        return jsonResponse({
          success: false,
          error: isEN ? 'Could not extract LinkedIn profile data. Please check the URL is correct and the profile is public, or upload your profile PDF.' : 'Não foi possível extrair dados do perfil LinkedIn. Verifica se o URL está correto e o perfil é público, ou faz upload do PDF do teu perfil.'
        }, 400);
      }
      try {
...
- NEVER make generic comments that could apply to any profile. Every sentence must be specific to this professional.`;
        const roastPrompt = isEN ? roastPromptEN : roastPromptPT;
        const geminiUrl = `https://generativelanguage.googleapis.com/v1beta/models/gemini-3-flash-preview:generateContent?key=${GEMINI_API_KEY}`;
        const geminiResponse = await fetch(geminiUrl, {
          method: 'POST',
          headers: {
