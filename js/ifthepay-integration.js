...
// CORRIGIDO: Backend próprio como fallback (URL corrigida)
 ...
 * Melhorado para mostrar +351 visualmente mas enviar formato correto para API
 */
...
/**
 * CORRIGIDO: Formata telefone para exibição visual com +351
 */
function formatPhoneDisplay(phone) {
    if (!phone) return '';
    
    const cleanPhone = formatPhoneForMBWay(phone);
    
    if (cleanPhone.startsWith('351') && cleanPhone.length >= 12) {
        return `+351 ${cleanPhone.substring(3, 6)} ${cleanPhone.substring(6, 9)} ${cleanPhone.substring(9)}`;
    }
...
console.log('IfthenPay Integration carregada - Versão Corrigida Final');
