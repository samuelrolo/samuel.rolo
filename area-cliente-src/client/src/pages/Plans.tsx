 ...
 *   • CTA diferenciado: "Upgrade" vs "Subscrever"
 */
              ...
              >
                ...
                {current ? (
                  ...
                  <button onClick={() => handleSubscribe(tc.tier)}
                    ...
                    {!user ? t('sub.createToSubscribe') : upgrade ? `Upgrade para ${t(`sub.${tc.tier}`)}` : t('sub.subscribe')}
            ...
            {faqs.map((faq, i) => (
              <div key={i} className="border border-[#e5e5e5] rounded overflow-hidden">
                <button onClick={() => setOpenFaq(openFaq === i ? null : i)}
                  ...
                  {openFaq === i ? <ChevronUp className="w-4 h-4 text-[#999] shrink-0" /> : <ChevronDown className="w-4 h-4 text-[#999] shrink-0" />}
                </button>
                {openFaq === i && (
