 ...
 *   • tabs renomeadas e reordenadas por frequência de uso
...
import EmptyAnalyses from '@/components/EmptyAnalyses';
import {
...
export default function Dashboard() {
  ...
  const tabs = useMemo<{ key: TabId; label: string; icon: typeof Wrench }[]>(() => {
    const base: { key: TabId; label: string; icon: typeof Wrench }[] = [
      ...
      { key: 'analyses', label: 'Análises',       icon: FileSearch },
    ];
    if (isSubscriber) base.push({ key: 'resources', label: 'Recursos', icon: BookOpen });
    base.push(
      { key: 'progress',     label: 'Progresso',  icon: BarChart3 },
      { key: 'profile',      label: 'Perfil',      icon: FileText },
      { key: 'subscription', label: 'Subscrição',  icon: CalendarClock },
    );
    return base;
  }, [isSubscriber]);
  ...
  // ── Resources ────────────────────────────────────────────────────────────────
  const resources = [
    { id: 'ebook-cv', title: 'CV Vencedor — guia completo', desc: 'Cria um CV que passa nos filtros ATS', type: 'PDF', size: '155 KB', url: 'https://d2xsxph8kpxj0f.cloudfront.net/105354394/92yTmUfG3DeUMDKSZxzXKb/Ebook_Como_Criar_um_CV_Vencedor_861d8b44.pdf', icon: FileText },
    { id: 'energia',  title: 'Energia para Liderar',         desc: 'Livro sobre liderança e energia',     type: 'PDF', size: '23 MB',  url: 'https://cvlumvgrbuolrnwrtrgz.supabase.co/storage/v1/object/public/member-resources/Energia_para_Liderar_Premium.pdf', icon: BookOpen },
    { id: 'linkedin', title: '10 Erros no LinkedIn',          desc: 'Os erros mais comuns e como evitá-los', type: 'PDF', size: '109 KB', url: 'https://cvlumvgrbuolrnwrtrgz.supabase.co/storage/v1/object/public/member-resources/10-erros-linkedin.pdf', icon: Linkedin },
    { id: 'script',   title: 'Script de Entrevistas',         desc: 'Guião completo para entrevistas',     type: 'PDF', size: '165 KB', url: 'https://cvlumvgrbuolrnwrtrgz.supabase.co/storage/v1/object/public/member-resources/Script_de_Entrevistas_Share2Inspire.pdf', icon: FileSearch },
  ];
  ...
  function getAnalysisSummary(analysis: SavedAnalysis): string {
    ...
    if (analysis.analysis_type === 'cv_analyser') {
      ...
      if (data.results_html) return data.results_html.replace(/<[^>]*>/g, ' ').replace(/\s+/g, ' ').trim().substring(0, 80) + '...';
    }
    ...
    if (analysis.analysis_type === 'career_path' && data.career_path?.title) return data.career_path.title;
    if (analysis.analysis_type === 'career_energy' && data.total_score) return `Score: ${data.total_score}${data.level ? ` — ${data.level}` : ''}`;
          ...
          <div className="space-y-5">
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
              {ALL_TOOLS.map(tool => {
                ...
                const quotaLabel = (() => {
                  if (isLocked) return `A partir de ${tool.minTier}`;
                  ...
                  return tool.quota === 'bonus' ? 'Bonus incluído' : 'Incluído no plano';
                })();
                ...
                return (
                  <div key={tool.type}
                    ...
                    {!isLocked && count > 0 && (
                      <div className="absolute top-3 right-3 px-2 py-0.5 bg-gold/8 border border-gold/15 rounded-full">
          ...
          <div className="space-y-6">
            {loadingAnalyses ? (
              <div className="flex items-center justify-center py-16">
                ...
                <span className="ml-2 text-sm text-[#999] font-light">A carregar...</span>
              </div>
            ) : analyses.length === 0 ? (
              <EmptyAnalyses
                variant={isSubscriber ? 'no-data' : 'no-sub'}
                onNavigatePlans={() => navigate('/planos')}
              />
            ) : (
              <>
                <div className="flex items-center justify-between">
                  <p className="text-sm font-medium text-[#1a1a1a]">
                    {analyses.length} {analyses.length === 1 ? 'análise guardada' : 'análises guardadas'}
                  </p>
                ...
                <div className="border border-[#e5e5e5] rounded-lg p-5">
                  ...
                  {cvUploaded && <p className="text-xs text-gold mt-2 font-light">CV carregado com sucesso</p>}
            ...
            {isSubscriber && subscription ? (
              <>
                ...
                <div className="flex items-center justify-between p-4 border border-[#e5e5e5] rounded-lg">
                  <div>
                    <p className="text-sm font-medium text-[#1a1a1a]">Área de Membro</p>
              ...
              <div className="border border-[#e5e5e5] rounded-lg p-6">
                <p className="text-sm font-medium text-[#1a1a1a] mb-1">Sem subscrição ativa</p>
