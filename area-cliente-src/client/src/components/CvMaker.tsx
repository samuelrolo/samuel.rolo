...
// ─── Template Configs ───────────────────────────────────────────────────────
const TEMPLATES: Record<CvTemplate, { label: string; labelEn: string; accent: string; bg: string; headerBg: string; sectionColor: string; textColor: string; subtextColor: string; borderColor: string; chipBg: string; chipText: string }> = {
  classic: {
    label: 'Clássico', labelEn: 'Classic',
  ...
  executive: {
    label: 'Executivo', labelEn: 'Executive',
...
export default function CvMaker({ lang, supabaseUrl, supabaseAnonKey, hyperTaskUrl, profileName, userId }: CvMakerProps) {
  ...
  const sendMessage = useCallback(async (overrideMsg?: string) => {
    ...
    try {
      ...
      if (data.success && data.reply) {
        ...
        setMessages(prev => [...prev, { role: 'assistant', content: pt ? 'Desculpa, ocorreu um erro. Tenta novamente.' : 'Sorry, an error occurred. Please try again.' }]);
      }
    } catch (err) {
      setMessages(prev => [...prev, { role: 'assistant', content: pt ? 'Erro de ligação. Tenta novamente.' : 'Connection error. Please try again.' }]);
    } finally {
      setChatLoading(false);
    }
  }, [chatInput, messages, cvData, lang, hyperTaskUrl, supabaseAnonKey, profileName, userId, pt]);
  ...
  const generatePdf = async () => {
    ...
    try {
      ...
      if (cvData.summary?.trim()) {
        addSection(pt ? 'Resumo Profissional' : 'Professional Summary');
        doc.setFontSize(9.5);
        doc.setFont('helvetica', 'normal');
      ...
      if (cvData.experiences && cvData.experiences.length > 0) {
        addSection(pt ? 'Experiência Profissional' : 'Professional Experience');
      ...
      if (cvData.education && cvData.education.length > 0) {
        addSection(pt ? 'Formação Académica' : 'Education');
      ...
      if (cvData.skills && cvData.skills.length > 0) {
        addSection(pt ? 'Competências' : 'Skills');
        doc.setFontSize(9.5);
        doc.setFont('helvetica', 'normal');
        doc.setTextColor(t.textColor);
      ...
      if (cvData.languages && cvData.languages.length > 0) {
        addSection(pt ? 'Idiomas' : 'Languages');
        for (const l of cvData.languages) {
          checkPage(6);
          doc.setFontSize(9.5);
          doc.setFont('helvetica', 'normal');
          doc.setTextColor(t.textColor);
          const langText = `${l.language} — ${l.level}${l.native ? (pt ? ' (Nativo)' : ' (Native)') : ''}${l.certificate ? ` | ${l.certificate}` : ''}`;
      ...
      if (cvData.certifications && cvData.certifications.length > 0) {
        addSection(pt ? 'Certificações' : 'Certifications');
      ...
      doc.setFontSize(7);
      doc.setTextColor('#CCCCCC');
      doc.text(`${pt ? 'Gerado com' : 'Generated with'} Share2Inspire CV Maker`, marginL, pageH - 8);

      const fileName = `CV_${(name || 'documento').replace(/\s+/g, '_')}_${new Date().toISOString().slice(0, 10)}.pdf`;
      doc.save(fileName);
    } catch (err) {
  ...
  const renderTabBar = () => (
    <div className="flex items-center border-b border-[#e5e5e5] mb-3">
      {([
        { key: 'chat' as CvTab, icon: MessageSquare, label: pt ? 'Chat IA' : 'AI Chat' },
        { key: 'form' as CvTab, icon: Edit3, label: pt ? 'Formulário' : 'Form' },
        { key: 'preview' as CvTab, icon: Eye, label: 'Preview' },
      ]).map(tab => (
        <button
          key={tab.key}
  ...
  const renderTemplateSelector = () => (
    <div className="flex items-center gap-2 mb-3 px-1">
      ...
      <span className="text-[10px] text-[#999] font-medium uppercase tracking-wider">{pt ? 'Template' : 'Template'}:</span>
    ...
    <div className="flex flex-col" style={{ minHeight: '380px', maxHeight: '520px' }}>
        ...
        {messages.length === 0 && (
          <div className="flex flex-col items-center justify-center py-8 text-center px-4">
            ...
            <p className="text-xs text-[#999] mb-4 px-2 max-w-md">
              {pt ? 'Constrói o teu CV passo a passo com ajuda da IA. Cola o teu link do LinkedIn para importar automaticamente ou começa do zero.' : 'Build your CV step by step with AI help. Paste your LinkedIn link to auto-import or start from scratch.'}
            </p>
            <div className="flex flex-col sm:flex-row gap-2">
              <button onClick={() => sendMessage(pt ? 'Olá, quero começar a construir o meu CV.' : 'Hi, I want to start building my CV.')} className="px-5 py-2.5 rounded-xl text-white text-sm font-medium transition-all hover:opacity-90 bg-gradient-to-r from-[#BFA14A] to-[#8F7A3A]">
                {pt ? 'Começar a construir CV' : 'Start building CV'}
              </button>
            </div>
          </div>
        )}
      ...
      <div className="flex items-center gap-2">
        <input
          ...
          placeholder={pt ? 'Descreve a tua experiência, formação ou cola o teu LinkedIn...' : 'Describe your experience, education or paste your LinkedIn...'}
  ...
  const renderForm = () => (
    <div className="space-y-1 overflow-y-auto pr-1" style={{ maxHeight: '520px' }}>
      {/* Personal Info */}
      <SectionHeader icon={User} title={pt ? 'Dados Pessoais' : 'Personal Info'} isOpen={openSections.personal} onToggle={() => toggleSection('personal')} />
      {openSections.personal && (
        <div className="pl-6 pr-2 pb-3 space-y-2.5">
          <div className="grid grid-cols-2 gap-2.5">
            <div><label className={labelClass}>{pt ? 'Nome Completo' : 'Full Name'}</label><input className={inputClass} value={cvData.personal_info?.name || cvData.personal_info?.full_name || ''} onChange={e => updatePersonalInfo('name', e.target.value)} /></div>
            <div><label className={labelClass}>{pt ? 'Cargo-alvo' : 'Target Role'}</label><input className={inputClass} value={cvData.personal_info?.target_role || cvData.target_role || ''} onChange={e => { updatePersonalInfo('target_role', e.target.value); setCvData(prev => ({ ...prev, target_role: e.target.value })); }} /></div>
          </div>
          <div className="grid grid-cols-2 gap-2.5">
            ...
            <div><label className={labelClass}>{pt ? 'Telefone' : 'Phone'}</label><input className={inputClass} value={cvData.personal_info?.phone || ''} onChange={e => updatePersonalInfo('phone', e.target.value)} /></div>
          </div>
          <div className="grid grid-cols-2 gap-2.5">
            <div><label className={labelClass}>{pt ? 'Localização' : 'Location'}</label><input className={inputClass} value={cvData.personal_info?.location || ''} onChange={e => updatePersonalInfo('location', e.target.value)} /></div>
      ...
      {/* Summary */}
      <SectionHeader icon={FileText} title={pt ? 'Apresentação / Resumo' : 'Summary'} isOpen={openSections.summary} onToggle={() => toggleSection('summary')} />
      {openSections.summary && (
        <div className="pl-6 pr-2 pb-3">
          <textarea
            ...
            placeholder={pt ? 'Escreve um resumo profissional...' : 'Write a professional summary...'}
          />
        </div>
      )}

      {/* Experience */}
      <SectionHeader icon={Briefcase} title={pt ? 'Experiência Profissional' : 'Work Experience'} isOpen={openSections.experience} onToggle={() => toggleSection('experience')} count={cvData.experiences?.length} />
      {openSections.experience && (
        <div className="pl-6 pr-2 pb-3 space-y-4">
          {(cvData.experiences || []).map((exp, i) => (
            <div key={i} className="border border-[#e5e5e5] rounded-lg p-3 relative group">
              ...
              <div className="grid grid-cols-2 gap-2 mb-2">
                <div><label className={labelClass}>{pt ? 'Cargo' : 'Role'}</label><input className={inputClass} value={exp.role || ''} onChange={e => updateExperience(i, 'role', e.target.value)} /></div>
                <div><label className={labelClass}>{pt ? 'Empresa' : 'Company'}</label><input className={inputClass} value={exp.company || ''} onChange={e => updateExperience(i, 'company', e.target.value)} /></div>
              </div>
              <div className="grid grid-cols-2 gap-2 mb-2">
                <div><label className={labelClass}>{pt ? 'Período' : 'Period'}</label><input className={inputClass} value={exp.period || exp.date_period || ''} onChange={e => updateExperience(i, 'period', e.target.value)} placeholder="Jan 2020 - Present" /></div>
                <div><label className={labelClass}>{pt ? 'Local' : 'Location'}</label><input className={inputClass} value={exp.location || ''} onChange={e => updateExperience(i, 'location', e.target.value)} /></div>
              </div>
              <label className={labelClass}>{pt ? 'Realizações' : 'Achievements'}</label>
              ...
              <button onClick={() => addBulletPoint(i)} className="text-[11px] text-[#BFA14A] hover:text-[#8F7A3A] font-medium flex items-center gap-1 mt-1"><Plus className="w-3 h-3" />{pt ? 'Adicionar realização' : 'Add achievement'}</button>
            </div>
          ))}
          <button onClick={addExperience} className="w-full py-2 rounded-lg border-2 border-dashed border-[#e5e5e5] text-sm text-[#999] hover:border-[#BFA14A] hover:text-[#BFA14A] transition-all flex items-center justify-center gap-1.5">
            <Plus className="w-4 h-4" />{pt ? 'Adicionar experiência' : 'Add experience'}
          </button>
        </div>
      )}

      {/* Education */}
      <SectionHeader icon={GraduationCap} title={pt ? 'Formação Académica' : 'Education'} isOpen={openSections.education} onToggle={() => toggleSection('education')} count={cvData.education?.length} />
        ...
        <div className="pl-6 pr-2 pb-3 space-y-3">
          {(cvData.education || []).map((edu, i) => (
            <div key={i} className="border border-[#e5e5e5] rounded-lg p-3 relative group">
              ...
              <div className="grid grid-cols-2 gap-2 mb-2">
                <div><label className={labelClass}>{pt ? 'Instituição' : 'Institution'}</label><input className={inputClass} value={edu.institution || ''} onChange={e => updateEducation(i, 'institution', e.target.value)} /></div>
                <div><label className={labelClass}>{pt ? 'Grau' : 'Degree'}</label><input className={inputClass} value={edu.degree || ''} onChange={e => updateEducation(i, 'degree', e.target.value)} /></div>
              </div>
              <div className="grid grid-cols-3 gap-2">
                <div><label className={labelClass}>{pt ? 'Área' : 'Field'}</label><input className={inputClass} value={edu.field || ''} onChange={e => updateEducation(i, 'field', e.target.value)} /></div>
                <div><label className={labelClass}>{pt ? 'Período' : 'Period'}</label><input className={inputClass} value={edu.period || edu.year || ''} onChange={e => updateEducation(i, 'period', e.target.value)} /></div>
                <div><label className={labelClass}>{pt ? 'Nota' : 'Grade'}</label><input className={inputClass} value={edu.grade || ''} onChange={e => updateEducation(i, 'grade', e.target.value)} /></div>
              </div>
            </div>
          ))}
          <button onClick={addEducation} className="w-full py-2 rounded-lg border-2 border-dashed border-[#e5e5e5] text-sm text-[#999] hover:border-[#BFA14A] hover:text-[#BFA14A] transition-all flex items-center justify-center gap-1.5">
            <Plus className="w-4 h-4" />{pt ? 'Adicionar formação' : 'Add education'}
          </button>
        </div>
      )}

      {/* Skills */}
      <SectionHeader icon={Star} title={pt ? 'Competências' : 'Skills'} isOpen={openSections.skills} onToggle={() => toggleSection('skills')} count={cvData.skills?.length} />
      {openSections.skills && (
        <div className="pl-6 pr-2 pb-3">
          ...
          <div className="flex gap-2">
            <input className={`${inputClass} flex-1`} value={newSkill} onChange={e => setNewSkill(e.target.value)} onKeyDown={e => { if (e.key === 'Enter') { e.preventDefault(); addSkill(); } }} placeholder={pt ? 'Nova competência...' : 'New skill...'} />
      ...
      {/* Languages */}
      <SectionHeader icon={Languages} title={pt ? 'Idiomas' : 'Languages'} isOpen={openSections.languages} onToggle={() => toggleSection('languages')} count={cvData.languages?.length} />
        ...
        <div className="pl-6 pr-2 pb-3">
          {(cvData.languages || []).map((l, i) => (
            <div key={i} className="flex items-center gap-2 mb-2 text-sm">
              ...
              {l.native && <span className="text-[10px] px-1.5 py-0.5 rounded bg-[#BFA14A]/10 text-[#BFA14A]">{pt ? 'Nativo' : 'Native'}</span>}
          ...
          <div className="flex gap-2 items-end mt-2">
            <div className="flex-1"><label className={labelClass}>{pt ? 'Idioma' : 'Language'}</label><input className={inputClass} value={newLang.language} onChange={e => setNewLang(prev => ({ ...prev, language: e.target.value }))} /></div>
            <div className="w-20"><label className={labelClass}>{pt ? 'Nível' : 'Level'}</label>
              <select className={inputClass} value={newLang.level} onChange={e => setNewLang(prev => ({ ...prev, level: e.target.value }))}>
                <option value="A1">A1</option><option value="A2">A2</option><option value="B1">B1</option><option value="B2">B2</option><option value="C1">C1</option><option value="C2">C2</option><option value="Nativo">Nativo</option>
              </select>
            </div>
      ...
      {/* Certifications */}
      <SectionHeader icon={Award} title={pt ? 'Certificações' : 'Certifications'} isOpen={openSections.certifications} onToggle={() => toggleSection('certifications')} count={cvData.certifications?.length} />
      {openSections.certifications && (
        <div className="pl-6 pr-2 pb-3">
          ...
          <div className="flex gap-2 items-end mt-2">
            <div className="flex-1"><label className={labelClass}>{pt ? 'Nome' : 'Name'}</label><input className={inputClass} value={newCert.name} onChange={e => setNewCert(prev => ({ ...prev, name: e.target.value }))} /></div>
            <div className="flex-1"><label className={labelClass}>{pt ? 'Entidade' : 'Issuer'}</label><input className={inputClass} value={newCert.issuer || ''} onChange={e => setNewCert(prev => ({ ...prev, issuer: e.target.value }))} /></div>
            <div className="w-28"><label className={labelClass}>{pt ? 'Data' : 'Date'}</label><input className={inputClass} value={newCert.date || ''} onChange={e => setNewCert(prev => ({ ...prev, date: e.target.value }))} placeholder="2024" /></div>
    ...
    if (!hasCvData) {
      return (
        <div className="flex flex-col items-center justify-center py-12 text-center">
          ...
          <p className="text-sm text-[#999]">{pt ? 'Preenche os dados no chat ou formulário para ver o preview.' : 'Fill in data via chat or form to see the preview.'}</p>
        </div>
      );
    }
      ...
      <div className="overflow-y-auto" style={{ maxHeight: '540px' }}>
        ...
        <div className="rounded-xl overflow-hidden shadow-md border" style={{ borderColor: t.borderColor, backgroundColor: t.bg }}>
          ...
          <div className="px-5 py-4 space-y-4">
            ...
            {cvData.summary?.trim() && (
              <div>
                <h3 className="text-[10px] font-bold uppercase tracking-wider mb-1.5" style={{ color: t.sectionColor }}>{pt ? 'Resumo Profissional' : 'Professional Summary'}</h3>
            ...
            {cvData.experiences && cvData.experiences.length > 0 && (
              <div>
                <h3 className="text-[10px] font-bold uppercase tracking-wider mb-1.5" style={{ color: t.sectionColor }}>{pt ? 'Experiência Profissional' : 'Professional Experience'}</h3>
                <div className="w-8 h-0.5 mb-2" style={{ backgroundColor: t.sectionColor }} />
                {cvData.experiences.map((exp, i) => (
            ...
            {cvData.education && cvData.education.length > 0 && (
              <div>
                <h3 className="text-[10px] font-bold uppercase tracking-wider mb-1.5" style={{ color: t.sectionColor }}>{pt ? 'Formação Académica' : 'Education'}</h3>
                <div className="w-8 h-0.5 mb-2" style={{ backgroundColor: t.sectionColor }} />
                {cvData.education.map((edu, i) => (
            ...
            {cvData.skills && cvData.skills.length > 0 && (
              <div>
                <h3 className="text-[10px] font-bold uppercase tracking-wider mb-1.5" style={{ color: t.sectionColor }}>{pt ? 'Competências' : 'Skills'}</h3>
                <div className="w-8 h-0.5 mb-2" style={{ backgroundColor: t.sectionColor }} />
            ...
            {cvData.languages && cvData.languages.length > 0 && (
              <div>
                <h3 className="text-[10px] font-bold uppercase tracking-wider mb-1.5" style={{ color: t.sectionColor }}>{pt ? 'Idiomas' : 'Languages'}</h3>
                <div className="w-8 h-0.5 mb-2" style={{ backgroundColor: t.sectionColor }} />
                <div className="flex flex-wrap gap-x-4 gap-y-1">
                  {cvData.languages.map((l, i) => (
                    <span key={i} className="text-[10px]" style={{ color: t.textColor }}>
                      <strong>{l.language}</strong> — {l.level}{l.native ? (pt ? ' (Nativo)' : ' (Native)') : ''}
                    </span>
                  ))}
                </div>
              </div>
            ...
            {cvData.certifications && cvData.certifications.length > 0 && (
              <div>
                <h3 className="text-[10px] font-bold uppercase tracking-wider mb-1.5" style={{ color: t.sectionColor }}>{pt ? 'Certificações' : 'Certifications'}</h3>
                <div className="w-8 h-0.5 mb-2" style={{ backgroundColor: t.sectionColor }} />
                {cvData.certifications.map((c, i) => (
          ...
          <div className="px-5 py-2 border-t text-center" style={{ borderColor: t.borderColor }}>
            <p className="text-[8px]" style={{ color: t.subtextColor }}>{pt ? 'Gerado com' : 'Generated with'} Share2Inspire CV Maker</p>
          </div>
        </div>
      </div>
    );
