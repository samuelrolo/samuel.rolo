          ...
          <div className="space-y-2">
            ...
            <div className="grid grid-cols-2 gap-2">
              {getATSSystems().map((s, i) => (
                <div key={i} className="p-2.5 rounded-lg bg-muted/20 border border-border flex justify-between items-center gap-2">
                  <span className="text-xs font-medium text-foreground shrink-0">{s.name}</span>
                  <span className={`text-xs font-semibold ${s.color} shrink-0`}>{s.compat}</span>
                </div>
              ))}
            </div>
          </div>
