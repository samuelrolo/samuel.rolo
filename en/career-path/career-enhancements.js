...
(function() {
  ...
  function createLinkedInButton(roleTitle) {
    ...
    const html = `<a href="${url}" target="_blank" rel="noopener" class="s2i-linkedin-mini" title="View jobs on LinkedIn">
      ${LINKEDIN_SVG} Jobs
    </a><span class="s2i-linkedin-note">active jobs on LinkedIn</span>`;
    return html;
  }
  ...
  function createCoursesSection(skills) {
    ...
    return `<div class="s2i-courses-section s2i-enhancement">
      <div class="s2i-courses-title">🎓 Recommended free courses</div>
    ...
    for (const el of allElements) {
      ...
      if (el.tagName && /^H[1-6]$/.test(el.tagName) && text.length > 5 && text.length < 150) {
        ...
        if (parent) {
          ...
          if (parentText.includes('%') && (parentText.includes('months') || parentText.includes('years') || parentText.includes('term') || parentText.includes('meses') || parentText.includes('anos'))) {
            roleCards.push({
              titleEl: el,
              titleText: text.trim(),
              containerEl: parent
